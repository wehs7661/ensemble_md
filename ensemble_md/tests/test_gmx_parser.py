...
def test_parse_log():
    ...
    wl_delta_1, weights_1, counts_1, equil_bool_1 = gmx_parser.parse_log(os.path.join(input_path, 'case2_1.log'))
    assert wl_delta_1 == 0.32
    assert weights_1 == [0.00000, 1.40453, 2.85258, 2.72480, 3.46220, 5.88607]
    assert counts_1 == [11, 13, 15, 4, 4, 50]
    assert equil_bool_1 is True
    ...
    wl_delta_2, weights_2, counts_2, equil_bool_2 = gmx_parser.parse_log(os.path.join(input_path, 'case2_2.log'))
    assert wl_delta_2 == 0.00099828
    assert weights_2 == [0.00000, 1.60419, 2.47705, 3.14963, 3.47840]
    assert counts_2 == [2, 3, 1, 2, 1]
    assert equil_bool_2 is True

    # Here we test Case 3
    wl_delta_3, weights_3, counts_3, equil_bool_3 = gmx_parser.parse_log(os.path.join(input_path, 'case3.log'))
