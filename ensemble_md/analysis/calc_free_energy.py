...
def calculate_free_energy(data, state_ranges, method="MBAR"):
    ...
    ----------
    data : pd.Dataframe
        A list of dHdl or u_nk data obtained from all replicas of the EEXE simulation of interest. 
    ...
    -------
    df : list
        A free energy profile with each entry averaged over replicas.
    df_err : list
        A list of err

    estimator : alchemlyb object
        An object of the corresponding class of alchemlyb.estimators that one can use to retrieve the 
        free energy profile and its uncertainty.
    """
    n_sim = len(data)
    ...
    df_list = [list(np.array(estimators[i].delta_f_)[:-1, 1:].diagonal()) for i in range(n_sim)]


    df_list = [list(estimators[i].delta_f_.iloc[0]) for i in range(n_sim)]
    errs = [list(estimators[i].d_delta_f_.iloc[0]) for i in range(n_sim)]

    return estimators, df_list, errs
