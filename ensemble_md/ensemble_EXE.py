...
class EnsembleEXE:
    ...
    def run_grompp(self, n, swap_pattern):
        ...
        """
        # (iteration_n, mdp_filename, gro_filename, swap_pattern, optional_grompp_args) -> new_tpr

        args_list = []
        for i in range(self.n_sim):  # for each ensemble member
    ...
    def run_mdrun(self, n):
        ...
        n : int
            The iteration index (starting from 0).
        """
        # (optional_runtime_args, (ensemble_size, current_rank), iteration_number) -> returncode, final_coordinates, dhdl_files, md_log
        # Note that these outputs effectively warrant capturing the mdrun working directory artifact.
    ...
    def run_EEXE(self, n, swap_pattern=None):
        ...
        # (optional_runtime_args, (ensemble_size, current_rank), iteration_number) -> returncode, final_coordinates, dhdl_files, md_log
        # Note that these outputs include several arbitrary files from the simulator
        # that we cannot easily identify through a gromacs API, so we need to just
        # plan to use the mdrun task directory artifact.
        self.run_mdrun(n)

        # 2nd synchronizing point for all MPI processes: To make sure no rank will start getting to the next
