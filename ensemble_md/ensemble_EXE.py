...
from  ensemble_md.exceptions import *
...
class EnsembleEXE:
    def __init__(self, yml_file):
        ...
        outfile  (str): The file name of the log file for documenting how different replicas interact during the process.
        """
        ...
        for i in required_args:
            if hasattr(self, i) is False:
                raise ParameterError(f"Required parameter '{i}' not specified in {yml_file}.")        
        ...
        self.n_tot = len(self.template['vdw-lambdas'])             # Total # of states. n_tot = n_sub * n_sim - (n_overlap) * (n_sum - 1), where n_overlap = n_sub - s
        self.n_sub = self.n_tot - self.s * (self.n_sim - 1)   # Number of states of each replica (assuming the same for all rep)
        self.state_ranges = [set(range(i * self.s, i * self.s + self.n_sub)) for i in range(self.n_sim)]  # A list of sets of state indices
    ...
    def update_MDP(self, new_template, sim_idx, iter_idx, states, wl_delta, weights, equil_bools):
        ...
        equil_bools  (list): A list of boolean variables indicating whether the weights of the simulations have been equilibrated.
        ...
        MDP['nsteps'] = self.nst_sim 
        MDP['init-lambda-state'] = states[sim_idx] - sim_idx * self.s   # Note that states[j] is the global index, so we shift it back to the local index. 
    ...
    def map_lambda2state(self):
        ...
        for i in range(self.n_tot):
            # Note that the order of the lambda values in the vector is the same as the dataframe generated by extract_dhdl
            if 'coul-lambdas' in self.template:
                if 'restraint-lambdas' in self.template:
                    self.lambda_dict[(self.template['coul-lambdas'][i], self.template['vdw-lambdas'][i]), self.template['restraint_lambdas'][i]] = i
                else:
        ...
        self.lambda_ranges= [[list(self.lambda_dict.keys())[j] for j in self.state_ranges[i]] for i in range(len(self.state_ranges))]  
    ...
    def extract_final_dhdl_info(self, dhdl_files):
        ...
        for j in range(self.n_sim):
            ...
            print(f'  Simulation {j}: State {states[j]}, (coul, vdw) = {list(self.lambda_dict.keys())[list(self.lambda_dict.values()).index(states[j])]}')
    ...
    def extract_final_log_info(self, log_files):
        ...
        equil_bools (list): A list of boolean variables indicating whether the weights were equilibrated in the simulation.
        """  
    ...
    def propose_swaps(self):
        ...
        if self.n_pairs > n_pairs_max:
            print(f"\nThe parameter `n_pairs` specified in the YAML file (n_pairs = {self.n_pairs}) exceeds the maximal number of simulation pairs that can be exchanged ({n_pairs_max}).")
        ...
        for i in range(self.n_pairs):
            ...
            swappables = [i for i in all_pairs if self.state_ranges[i[0]].intersection(self.state_ranges[i[1]]) != set()]
    ...
    def calc_prob_acc(self, swap, dhdl_files, states, lambda_vecs, weights):
        ...
        states       (list): A list of last sampled states of ALL simulaitons. Typically generated by extract_final_dhdl_info.
        lambda_vecs  (list): A list of lambda vectors corresponding to the last sampled states of ALL simulations. Typically generated by extract_final_dhdl_info.
        weights      (list): A list of lists of final weights of ALL simulations. Typiecally generated by extract_final_log_info.
        ...
        if states[swap[0]] not in self.state_ranges[swap[1]] or states[swap[1]] not in self.state_ranges[swap[0]]:
            ...
            if self.mc_scheme == 'same_state':
                ...
                new_state_0 = self.lambda_ranges[swap[0]].index(new_lambda_0)    # new state index (local index in simulation swap[0])
                new_state_1 = self.lambda_ranges[swap[1]].index(new_lambda_1)    # new state index (local index in simulation swap[1])
...
def run_EEXE(n_sim, n, parallel=True):
    ...
    if rank == 0:
        print(f'Running an ensemble of simulations ...\n')
    ...
    if parallel is True:
        ...
        # Note that we could use output_files argument to customize the output file names but here we'll just use the defaults.
