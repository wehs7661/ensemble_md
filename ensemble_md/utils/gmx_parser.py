...
def parse_log(log_file):
    ...
    This include the final Wang-Landau incrementor (:code:`wl_delta`), the final list of
    lambda weights (:code:`weights`), the final histogram counts (:code:`counts`), the
    time (in ps) it took to equilibrate the weights (:code:`equil_time`) and the list of
    time frames when the weights were updated (:code:`t_update`). Typically, there are three
    types of log files from an expanded ensemble simulation:

    - Case 1: The weights are still updating in the simulation and has never been equilibrated.
    ...
    weights, counts = [], []
    case_1_3, equil_time_found = False, False  # case_1_3: whether the simulation is cases 1 or 3
    ...
    for l in lines:  # noqa: E741
        ...
        if "lmc-stats" in l:
            if l.split("=")[1].split()[0] in ["no", "No"]:
                # Case 3: The weights are fixed. -> could be equilibrated in the previous iteration
                fixed_bool = True
                equil_time = 0
                wl_delta = None
                wl_delta_found = True
